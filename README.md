...
| 模块 | 功能 |
|---|---|
| 🏠 首页 | Bento 问候主卡（每日一句 + 今日进度环）、天气卡（wttr.in 免密钥）、各模块统计、今日待办、近期日程、**追剧更新**、**📰 RSS 资讯流**（少数派/阮一峰）、快捷入口 |
...
| 📖 阅读 | TXT / EPUB 小说 + CBZ/ZIP 漫画导入（EPUB 元数据书名/作者/封面/nav·NCX 目录解析，TXT GBK/UTF-8 自动识别）、章节目录 + 全文搜索、书签、自动翻页、**阅读方式三选**（右开/左开/无缝滚动跨章连读）、中文排版缩进、**8 主题 + 自定义背景文字色**、6 字体、文字可选中复制、**在读中续读卡 + 连续天数 streak**、进度自动记忆 |
| 🔗 导航 | 分类网格书签、别名拼音搜索、哨兵存活检测（失效标红） |

**跨模块联动**：影视预约 → 日历事件；待办截止 → 日历事件；Trakt 追剧播出表 → 日历事件；首页聚合一切。

**安全模型**：浏览全公开；增删改需站主令牌（写操作被拒时右下角解锁卡输口令即恢复，10 年有效）。

**PWA**：`sw.js` 离线壳（hash 资源缓存优先 / 导航网络优先回落）+ `manifest.webmanifest`，可安装到桌面手机。

**年度 Wrapped**：首页入口 → `/wrapped` 故事式 7 幕年度回顾（影视/游戏/阅读/待办聚合）。
